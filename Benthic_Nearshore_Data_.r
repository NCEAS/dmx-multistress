...
ENSO_annual <- enso_df %>%
                    ...
                    ungroup()  # 
# 
BenNear <- merge(BenNear,ENSO_annual,all.x=T) 
...
pdo_annual <- pdo_df %>% 
              ...
              ungroup() 
#
BenNear <- merge(BenNear,pdo_annual,all.x=T)
...
npgo_annual <- npgo_df %>% 
               ...
               ungroup()  # 
#
BenNear <- merge(BenNear,npgo_annual,all.x=T) 
...
upanom <- upanom_df %>% 
          ...
          ungroup() 
#
BenNear <- merge(BenNear,upanom,all.x=T)
...
#
BenNear <- merge(BenNear,Phy,all.x=T)
...
head(SatChl_df)
#
BenNear <- merge(BenNear,SatChl_df,all.x=T)
...
#
BenNear <- merge(BenNear,SST,all.x=T)
